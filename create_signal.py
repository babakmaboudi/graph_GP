...
from prior import Matern_graph, Matern_graph_pytorch, edge_correlation, Matern_graph_pytorch_new
...
def edge_similarity(e1, e2):
    # Example: 1.0 if share a node, 0.5 if they’re in the same triangle, 0 otherwise
    shared_nodes = len(set(e1) & set(e2))
    return float(shared_nodes > 0)
...
def test_local_correlation():
    ...
    A = torch.zeros((len(edge_index), len(edge_index))).to(torch.float64)

    for e1 in L.nodes():
        i = edge_index[e1]
        for e2 in L.neighbors(e1):
            j = edge_index[e2]
            sim = edge_similarity(e1, e2)
            A[i, j] = sim

    # Normalize and apply
    A += torch.eye(len(A))  # Optional self-loop
    A = A / A.sum(1, keepdim=True)  # Normalize
    x_smooth = A @ x_true[:110]


    G.update_L( prior_map(x_smooth) , normalize=True) # updating the graph weights accroding to the unknown
    y_true = G.sample_stationary(v0, nu=nu_prior ) # creating a noise free signal
    ...
    # saving the signal file
    obs_data = {'porb_type': 'stationary', 'v0': v0, 'x_true': x_true, 'x_smooth': x_smooth, 'y_true': y_true, 'noise_vec': noise_vec, 'nu_prior': nu_prior, 'prior_map_mean': prior_map_mean, 'prior_map_scale': prior_map_scale, 'prior_map_text': prior_map_text}

    with open('./obs/torch/correlation/obs.pickle', 'wb') as handle:
        pickle.dump(obs_data, handle, protocol=pickle.HIGHEST_PROTOCOL)
    ...
    plotter = Graph_Plotter(graph, y_true.detach().numpy(), axes[0], pos=state_positions)
    plotter.plot_graph_wieghts(prior_map(x_smooth).detach().numpy(), axes[2])
...
def test_stationary():
    ...
    # defining a Matern prior on the graph
    G = Matern_graph_pytorch_new(graph)
    W = G.get_edge_weights()
    ...
    x_true = torch.randn(len(W))
    G.compute_laplacian_from_tensor_autograd( prior_map(x_true), normalized=True )

    dtype = torch.float64
    # defining initial nodal values
    #v0 = 10*torch.ones(G.N, dtype=dtype)
    #v0 = 1+torch.rand(G.N, dtype=dtype)
    v0 = torch.zeros(51, dtype=dtype)
    v0[25] = 10.
    #v0 = torch.abs( torch.randn(51,dtype=dtype) )
    #v0 = torch.ones(51, dtype=dtype)



    y_true = G.sample_stationary(v0, nu=nu_prior)
    ...
    with open('./obs/torch/correlation/stationary/obs.pickle', 'wb') as handle:
        pickle.dump(obs_data, handle, protocol=pickle.HIGHEST_PROTOCOL)
...
def test_heat():
    ...
    dtype = torch.float64
    # defining initial nodal values
    v0 = 0.2*torch.abs( 5*torch.randn(51, dtype=dtype) )
    #v0 = torch.zeros(51, dtype=dtype)
    #v0[25] = 10.

    # parameters used in the prior
    nu_prior = 1 # regularity
    dt_prior = 0.001 # time-step size
    ...
    # create a signal
    #x_true = torch.randn(G.num_edges).to(dtype) # the unknown for the inverse problem
    x_true = torch.randn( 110 ).to(dtype)
    #L = nx.line_graph(graph)
    ...
    with open('./obs/torch/correlation/heat/obs.pickle', 'wb') as handle:
        pickle.dump(obs_data, handle, protocol=pickle.HIGHEST_PROTOCOL)
