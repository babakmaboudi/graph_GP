...
def create_signal_stationary_linearreaction_pytorch():
    ...
    dir = './obs/stationary_linearreaction/'
    if not os.path.exists(dir):
            os.makedirs(dir)
...
def create_signal_stationary_nonlinearreaction_pytorch():
    # loading the graph of the US states
    graph = pickle.load(open('./data/covid_data/g.pkl', "rb"))
    ...
    y_true = G.sample_stationary_with_nonlinearreaction(v0, nu=nu_prior)  # creating a noise free signal
    ...
    # saving the signal file
    obs_data = {'porb_type': 'stationary', 'v0': v0, 'x_true': x_true, 'y_true': y_true, 'noise_vec': noise_vec,
                'nu_prior': nu_prior, 'prior_map_mean': prior_map_mean, 'prior_map_scale': prior_map_scale,
                'prior_map_text': prior_map_text}

    dir = './obs/stationary_nonlinearreaction/'
    if not os.path.exists(dir):
            os.makedirs(dir)
...
def create_signal_heat_nonlinearreaction_pytorch():
    ...
    dir = './obs/heat_nonlinearreaction/'
    if not os.path.exists(dir):
            os.makedirs(dir)
