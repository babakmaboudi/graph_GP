...
# NOTE: Each graph-inference map function also writes a top-30 edge-weight
# uncertainty plot using transformed posterior samples and an empirical 95% HDI.
import cartopy.io.shapereader as shpreader
import torch

# NOTE: Inference figures now use a quantitative HDI panel instead of
# a posterior-standard-deviation map. Synthetic cases include truth; real-COVID
# cases omit truth and use state-state connection labels.
STATE_ORDER = [
    "alabama",
    "alaska",
    "arizona",
...
def empirical_hdi(samples, prob=0.95):
    """
    Empirical shortest-width HDI along axis 0.

    Parameters
    ----------
    samples : array-like, shape (n_samples, n_parameters)
        Posterior samples already transformed to physical edge-weight space.
    prob : float
        Posterior mass contained in the HDI.

    Returns
    -------
    lower, upper : np.ndarray
        Lower and upper HDI endpoints for each edge.
    """
    ...
    if n_samples < 2:
        raise ValueError("Need at least two posterior samples to compute an HDI.")

    interval_count = int(np.floor(prob * n_samples))
    interval_count = min(max(interval_count, 1), n_samples - 1)

    sorted_samples = np.sort(samples, axis=0)
    widths = sorted_samples[interval_count:, :] - sorted_samples[:n_samples - interval_count, :]
    min_idx = np.argmin(widths, axis=0)

    cols = np.arange(n_parameters)
    lower = sorted_samples[min_idx, cols]
    upper = sorted_samples[min_idx + interval_count, cols]
    return lower, upper



def get_edge_connection_labels(graph):
    """
    Label each edge using the state ordering supplied in script.py.
    ...
    If graph nodes are already state-name strings, those names are used directly.
    Otherwise integer-like node IDs are interpreted as indices into STATE_ORDER.

    IMPORTANT: these labels are returned in graph.edges() iteration order, which
    must match the edge ordering used by the inferred weight vector.
    """
    def node_to_state(node):
        # If graph node itself is a recognizable state-name string, use it.
        if isinstance(node, str):
            node_clean = node.strip().lower()
            if node_clean in STATE_ORDER:
                return node_clean

            # Numeric strings such as "0", "1", ...
            try:
                idx = int(node_clean)
                if 0 <= idx < len(STATE_ORDER):
        ...
        # Integer / numpy-integer node IDs.
        try:
            idx = int(node)
            if 0 <= idx < len(STATE_ORDER):
                return STATE_ORDER[idx]
    ...
    return np.asarray(
        [f"{node_to_state(u)}-{node_to_state(v)}" for u, v in graph.edges()],
        dtype=object,
    )
...
def plot_top_edge_hdi_panel(
    ...
    edge_labels=None,
    title=None,
    annotate_edge_labels=False,
):
    """
    Quantitative top-k posterior summary drawn directly into an existing axis.

    Synthetic cases:
      top-k edges are selected by descending true physical edge weight.

    Real-data cases:
      top-k edges are selected by descending posterior mean because truth is unknown.

    Colors:
      blue   = posterior mean + HDI
      orange = truth
      green  = MAP

    For compact real-COVID figures, annotate_edge_labels=True writes the
    state-state connection vertically beside each HDI bar, while the x-axis
    retains only the edge index.
    """
    w_map = np.asarray(w_map).reshape(-1)
    w_samples = np.asarray(w_samples)

    if w_samples.ndim != 2:
        raise ValueError(
            f"Expected w_samples with shape (n_samples, n_edges), got {w_samples.shape}"
        )

    mean_all = np.mean(w_samples, axis=0)
    if mean_all.size != w_map.size:
        raise ValueError(
            f"Posterior sample edge count ({mean_all.size}) does not match MAP ({w_map.size})."
        )

    if show_truth:
        if w_true is None:
            raise ValueError("w_true must be supplied when show_truth=True.")
        w_true = np.asarray(w_true).reshape(-1)
        if w_true.size != mean_all.size:
            raise ValueError(
                f"Truth edge count ({w_true.size}) does not match posterior ({mean_all.size})."
            )
        ordering_values = w_true
    else:
        ordering_values = mean_all

    k = min(int(top_k), mean_all.size)
    selected = np.argsort(ordering_values)[::-1][:k]

    mean_sel = mean_all[selected]
    map_sel = w_map[selected]
    samples_sel = w_samples[:, selected]
    hdi_low, hdi_high = empirical_hdi(samples_sel, prob=hdi_prob)
    ...
    ax.errorbar(
        ...
        capsize=3,
        linewidth=1.1,
        label=rf"Posterior mean + {int(round(100*hdi_prob))}\% HDI",
        zorder=2,
    )
    ...
    # x-axis always reports edge index, as requested.
    ax.set_xticks(x)
    tick_labels = ax.set_xticklabels([str(int(i)) for i in selected], fontsize=8)

    # For dense top-30 panels, stagger alternating edge-index labels vertically
    # into two rows so neighboring labels remain readable without rotation.
    if k > 15:
        for j, label in enumerate(tick_labels):
            label.set_y(-0.015 if j % 2 == 0 else -0.075)
        ax.set_xlabel("Edge index", labelpad=15)
    else:
        ax.set_xlabel("Edge index", labelpad=2)
    ax.set_ylabel(r"Physical edge weight $w$", labelpad=2)

    if title is None:
        title = "(c) Top conductive edges"
    ax.set_title(title, fontsize=15)

    # For real-COVID cases, write state-state connection vertically next to
    # each HDI bar rather than consuming x-axis width.
    if annotate_edge_labels and edge_labels is not None:
        edge_labels = np.asarray(edge_labels, dtype=object)
        if edge_labels.size != mean_all.size:
            raise ValueError(
                f"edge_labels has length {edge_labels.size}, expected {mean_all.size}."
            )

        # Anchor every state-state label at the same vertical position,
        # just above the x-axis.  Use an axis-coordinate y position so the
        # label height does not depend on the HDI interval for that edge.
        for xpos, edge_idx in zip(x, selected):
            ax.text(
                xpos + 0.10,
                0.06,
                str(edge_labels[edge_idx]),
                rotation=90,
                va="bottom",
                ha="left",
                fontsize=6.5,
...
def plot_top_edge_hdi(
    ...
    """
    Plot selected physical edge weights with posterior HDI error bars.

    Synthetic cases:
        Edges are selected by descending true physical edge weight.

    Real-data cases (show_truth=False):
        Edges are selected by descending posterior mean because no true
        parameter is available.

    Color convention matches the 1D heat plots:
        blue   = posterior mean + HDI
        orange = truth
        green  = transformed MAP
    """
    w_map = np.asarray(w_map).reshape(-1)
    w_samples = np.asarray(w_samples)
    ...
    fig, ax = plt.subplots(figsize=(max(8.0, 0.34 * k), 4.6))

    # Same convention as the 1D heat figure.
    ax.errorbar(
        positions,
        mean_sel,
        yerr=yerr,
        ...
        capsize=3,
        linewidth=1.1,
        label=rf"Posterior mean and {int(round(100*hdi_prob))}\% HDI",
        zorder=2,
    )

    if show_truth:
        truth_sel = w_true[selected]
        ax.scatter(
            positions,
            truth_sel,
            color="tab:orange",
    ...
    if edge_labels is None:
        labels = [str(int(i)) for i in selected]
        xlabel = "Edge index (ordered from largest to smallest)"
    else:
        edge_labels = np.asarray(edge_labels, dtype=object)
        if edge_labels.size != mean_all.size:
            raise ValueError(
        ...
        labels = [str(edge_labels[i]) for i in selected]
        xlabel = "Connection (ordered from largest to smallest posterior mean)"

    ax.set_xticks(positions)
    ax.set_xticklabels(labels, rotation=90 if k > 8 else 35, ha="right", fontsize=8)
    ax.set_xlabel(xlabel)
    ax.set_ylabel(r"Physical edge weight $w$")

    if title is None:
        if show_truth:
            title = rf"Highest {k} edge weights with posterior HDI"
        else:
            title = rf"Highest {k} inferred edge weights with posterior HDI"
    ax.set_title(title)
...
def plot_MAP_stationary_linearreaction():
    ...
    w_true = prior_map(x_true).detach().numpy()
    w_MAP = prior_map(x_MAP).detach().numpy()
    ...
    m0 = plotter0.plot_graph_wieghts(w_MAP, ax=ax_map[0])
    ...
    # --- Quantitative top-10 edge posterior summary
    ax_cbar[2].set_visible(False)
    plot_top_edge_hdi_panel(
        ax=ax_map[2],
        w_true=w_true,
        w_map=w_MAP,
        w_samples=samples_pushed_forward.detach().cpu().numpy(),
        top_k=30,
        hdi_prob=0.95,
        show_truth=True,
        edge_labels=None,
        title=r'(c) Top 30 conductive edges',
    )


    # Ensure colorbar axes look clean
    for cax in ax_cbar[:2]:
        cax.yaxis.set_visible(False)
...
def plot_stationary_linearreaction_real_covid():
    ...
    # creating a forward operator
    graph = pickle.load(open('./data/covid_data/g.pkl', "rb")) # the graph containing the nodal information and the connections
    edge_connection_labels = get_edge_connection_labels(graph)
    G = Matern_graph_pytorch(graph) # Initiating a graph Gaussian process
    ...
    plot_top_edge_hdi_panel(
        ...
        hdi_prob=0.95,
        show_truth=False,
        edge_labels=edge_connection_labels,
        annotate_edge_labels=True,
        title=r'(c) Top 10 conductive edges',
    )
